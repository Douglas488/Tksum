...
本服务支持在生成总结表前要求输入谷歌验证器（TOTP）6 位动态码。

1. 在 Render 的 **Environment** 中新增变量：
   - **Key**：`TOTP_SECRET`
   - **Value**：谷歌验证器使用的 **Base32 密钥**（在添加账户时手动输入密钥即可得到，或从验证器应用导出）。
2. 未设置 `TOTP_SECRET` 时，`/api/verify-totp` 会返回 503，前端会提示「服务未配置验证」；设置后用户需在 index9 弹窗中输入当前 6 位验证码才能继续生成。
3. 验证码每 30 秒刷新一次，页面会显示「验证码有效期剩余 XX 秒」倒计时。
